-- =============================================================================
-- Migration 0013 — Persist orders_total in close_session_tx
--
-- WHY: sessions.orders_total was written once (to 0) at session start and
-- never updated, while report_revenue_by_day (migration 0007) reads
-- s.orders_total for the owner dashboard "Orders revenue" split. This caused
-- session-attached F&B revenue to appear as 0 in the dashboard breakdown and
-- broke the reconciliation invariant:
--   grand_total = time_total + orders_total − discount
-- The stored grand_total was already correct (computed fresh from order lines
-- before enqueue), but the orders_total COMPONENT column was dead. This
-- migration makes orders_total authoritative by writing it inside the same
-- atomic UPDATE that writes time_total and grand_total.
--
-- CHANGE: one additional column in the UPDATE … SET list inside
-- close_session_tx. The ONLY diff from migration 0009's definition is:
--   orders_total   = coalesce((p_session_patch->>'orders_total')::bigint, orders_total)
-- Added after grand_total. COALESCE makes the patch field optional: a client
-- that omits the key preserves the existing row value, making the change
-- backward-compatible (old client → 0 preserved; new client → writes correct
-- total). All other behavior, the function signature, idempotency guarantees,
-- security guards, and every other column are byte-for-byte identical to 0009.
--
-- RLS-NEUTRAL: this migration adds no new policy, no new WITH CHECK predicate,
-- and no new SECURITY DEFINER data path. The UPDATE already runs inside the
-- existing SECURITY DEFINER boundary, behind the same two explicit guards:
--   (1) p_tenant_id <> current_tenant_id() → 42501 (cross-tenant rejected)
--   (2) not is_active_member() → 42501 (unauthenticated / inactive member)
-- orders_total is tenant-owned data written within the already-guarded
-- transaction; the risk surface is unchanged.
--
-- SECURITY REVIEWER: sign-off required. The only data path that changed is
-- one additional SET column assignment inside the existing guarded UPDATE.
-- Verify that the tenant/member guards (lines 0 in the function body) are
-- identical to migration 0009 and that no new privilege escalation exists.
--
