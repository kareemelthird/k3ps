...
| **2. Tenant foundation** | ADR on isolation model; Supabase schema with `tenant_id`+`branch_id`; auth + JWT claim; multi-tenant RLS; super-admin tenant provisioning; build `@ps/core` (money/time/inventory/types) reusing the trial's sound algorithms | core, backend | ⬜ next |
| **3. Walking skeleton** | Thin end-to-end slice: login → tenant → branch → device → start/close one session, on mobile + web | all | ⬜ |
| **4. Devices + Sessions + Pricing** | Build the pricing engine in `@ps/core` (reusing the trial's sound math); live device grid; session lifecycle + segments (open/prepaid/fixed-match) | core, backend, mobile | ⬜ |
