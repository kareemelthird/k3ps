...
export default function TenantDetailPage({ params }: PageProps) {
  ...
  // Breadcrumb back link — RTL-aware: the arrow SVG rotates 180° in RTL so it
  // always points "backward" in the reading direction (right in RTL, left in LTR).
  const breadcrumb = (
    <Link
      href="/admin"
      className="inline-flex items-center gap-xs text-label text-text-muted hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-xs"
    >
      <svg
        aria-hidden="true"
        width="16"
